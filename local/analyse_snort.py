...
# Analyse a pcap file using Snort rules
                ...
                if rule["protocol"].upper() == pkt_proto:
                    ...
                    if rule["content"]:
                        ...
                        if isinstance(rule["content"], str) and rule["content"] not in pkt_payload:
                            continue
                        # Check if all content is in the payload
                    ...
                    alerts.append((pkt_ip, pkt_dst, pkt_proto, rule["msg"]))
...
alerts = analyse_pcap(pcap_file, rules)

with open("alerts.txt", "w") as f:
    for alert in alerts:
        f.write(" | ".join(map(str, alert)) + "\n")

print(f"Analyse done. {len(alerts)} alerts detected. Results in 'alerts.txt'.")
